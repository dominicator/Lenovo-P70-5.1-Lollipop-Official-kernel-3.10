...
int apk_debug_flag = 0; //0 for no apk upgrade, 1 for apk upgrade// add by zhaofei - 2014-02-22-15-12

int fts_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf,
			  u32 dw_lenth);

static unsigned char CTPM_FW[] = {
	#include "FT5436I_AL890_laibao_V0x1c_20150323_app.i"
};  
		    ...
		    int writelen, char *readbuf, int readlen)
{
	int ret,i;
		...
		struct i2c_msg msgs[] = {
			{
			 .addr = client->addr,
			 .flags = I2C_M_RD, normal I2c transfer
	if (writelen > 0) {
		struct i2c_msg msgs[] = {
			{
			 .addr = client->addr,
	...
	if(writelen!=0)
	{
		...
		else
		{
			...
			if((ret=i2c_master_send(client, (unsigned char *)I2CDMABuf_pa, writelen))!=writelen)
				dev_err(&client->dev, "###%s i2c write len=%x,dma addr = 0x%lx\n", __func__,ret,(unsigned long)I2CDMABuf_pa);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	fts_read_reg(client, FT_REG_FW_VER, &uc_tp_fm_ver);// read tp fw version to uc_tp_fm_ver by wangyang
	fts_read_reg(client, FT_REG_VENDOR_ID, &uc_tp_vendor_id);
	if(0x55 != uc_tp_vendor_id)
	{
	...
	const char __user *buff, unsigned long len, void *data)
{
	...
	case PROC_SET_TEST_FLAG:
		...
		printk("fts_debug_write apk_debug_flag=%d\n",apk_debug_flag);
		break;
	...
	const char __user *buff, unsigned long len, void *data)
{
	...
	TPD_DMESG("ctp:ft5x0x_debug_read++\n");
	switch (proc_operate_mode) {
	case PROC_UPGRADE:
		TPD_DMESG("ctp:case PROC_UPGRADE:\n");
		...
		break;
	case PROC_READ_REGISTER:
		TPD_DMESG("ctp:case PROC_READ_REGISTER:\n");
		readlen = 1;
...
{
	TPD_DMESG("ctp:ft5x0x_create_apk_debug_channel ++ \n");
